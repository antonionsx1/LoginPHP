...
if (isset($_POST["registrar"])) {
    ...
    $sql_existe_user = "SELECT idusuarios FROM usuarios WHERE usuario = '$user'";
    $res_existe_user = $conexion->query($sql_existe_user);
    $filas = $res_existe_user->num_rows;
    if ($filas > 0) {
        echo "<script> alert ('El usuario ya existe'); window.location = 'index.php' </script>";
    } else {
        $sql_new_user = "INSERT INTO usuarios(NombreC, Correo, usuario, password) VALUES ('$nombre', '$correo', '$user', '$pass_encriptada')";
        $res_nuevo_usuario = $conexion->query($sql_new_user);
        if ($res_nuevo_usuario > 0) {
